...
import { createTenantCrmRegistry, parseTenantCrmConnectors } from "./tenant-crm";

test("tenant CRM registry supports external and HerreB managed modes", () => {
  const registry = createTenantCrmRegistry(JSON.stringify([
    { tenantId: "herreb", mode: "HERREB_MANAGED_CRM", connectorId: "herreb-client0" },
    { tenantId: "client-a", mode: "EXTERNAL_CRM", connectorId: "hubspot-client-a" }
  ]));
...
test("tenant CRM connector configuration rejects duplicate tenant ownership", () => {
  assert.throws(() => parseTenantCrmConnectors([
...
test("malformed connector configuration fails closed", () => {
  ...
  assert.throws(() => registry.resolve("herreb"), /TENANT_CRM_CONNECTORS_INVALID/);
});
