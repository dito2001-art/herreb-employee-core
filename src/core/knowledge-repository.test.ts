...
test("knowledge repository isolates records by tenant", async () => {
  ...
  assert.deepEqual(
    (await repository.list("tenant-a")).map((item) => item.tenantId),
    ["tenant-a"]
  );
});
...
test("knowledge repository fails closed on cross-tenant writes", async () => {
  ...
  await assert.rejects(
    repository.put("tenant-a", record("tenant-b", "forbidden")),
    /TENANT_ISOLATION/
  );
...
test("knowledge repository scopes namespace queries inside tenant", async () => {
  ...
  assert.deepEqual(
    (await repository.list("tenant-a", "marketing")).map((item) => item.id),
    ["mkt"]
  );
});

test("verified canonical query excludes retrieved unverified and other tenants", async () => {
  const repository = createInMemoryTenantKnowledgeRepository();
  await repository.put("tenant-a", record("tenant-a", "verified"));
  await repository.put("tenant-a", {
    ...record("tenant-a", "retrieved"),
    kind: "RETRIEVED",
    verified: true
  });
  await repository.put("tenant-a", {
  ...
  await repository.put("tenant-b", record("tenant-b", "other"));

  assert.deepEqual(
    (await repository.listVerifiedCanonical("tenant-a", "marketing")).map(
      (item) => item.id
    ),
    ["verified"]
  );
