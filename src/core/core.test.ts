...
const baseContext = parseTenantContext({ tenantId: "tenant-a", employeeId: "EMP-001", workspaceId: "workspace-1", actorId: "fernando", channel: "test", correlationId: "run-1" });
...
test("policy is deny-by-default and employee boundaries are enforced", () => {
  assert.equal(authorizeCapability(baseContext, "unknown.capability").decision, "DENY");
...
test("adapter registry rejects unknown capabilities and invalid employee scopes", () => {
  ...
  assert.throws(() => registry.register({ id: "unknown", capabilities: ["unknown.capability"], employees: ["EMP-001"], async execute() { return { ok: true }; } }));
  assert.throws(() => registry.register({ id: "wrong-employee", capabilities: ["calendar.read"], employees: ["EMP-001"], async execute() { return { ok: true }; } }));
});

test("green capability executes without approval", async () => {
  const registry = createAdapterRegistry();
  const adapter: CapabilityAdapter = { id: "test-offering", capabilities: ["offering.read"], employees: ["EMP-001"], async execute() { return { ok: true, output: { offerings: [] }, evidence: { adapter: "test-offering" } }; } };
  registry.register(adapter);
  const result = await executeCapability(registry, { context: baseContext, capabilityId: "offering.read", input: {} });
