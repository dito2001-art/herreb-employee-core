...
test("tenant scoped keys include tenant employee and workspace", () => {
  assert.equal(
    ...
    "tenant-a:EMP-001:workspace-1:conversation:123"
  );
});

test("policy is deny-by-default", () => {
...
test("green capability executes without approval", async () => {
  ...
  const adapter: CapabilityAdapter = {
    ...
    employees: ["EMP-001"],
    async execute() {
      return { ok: true, output: { offerings: [] }, evidence: { adapter: "test-offering" } };
    }
  };
...
test("yellow capability requires explicit approval", async () => {
  const registry = createAdapterRegistry();
  registry.register({
    id: "test-quote",
  ...
  const approved = await executeCapability(
    registry,
    { context: baseContext, capabilityId: "quote.create", input: {} },
